...
import { API_BASE_URL, WP_API_ENDPOINT } from '../config';
...
export const BlogSection: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await fetch(`${WP_API_ENDPOINT}/posts?per_page=3`);
    ...
    <section id="blog" className="py-24 px-6 bg-[#F5F5F7] dark:bg-brand-black border-t border-slate-200 dark:border-white/5 transition-colors duration-500">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-end mb-12">
          ...
          <a href={API_BASE_URL} target="_blank" rel="noreferrer" className="hidden md:flex items-center gap-2 text-brand-periwinkle hover:text-slate-900 dark:hover:text-white transition-colors text-sm font-medium">
        ...
        {loading ? (
          ...
          <div className="text-center py-12 border border-slate-200 dark:border-white/5 rounded-2xl bg-white/5">
            ...
            <a href={API_BASE_URL} target="_blank" rel="noreferrer" className="text-brand-periwinkle mt-2 inline-block">Visit Blog directly</a>
          </div>
        ) : (
