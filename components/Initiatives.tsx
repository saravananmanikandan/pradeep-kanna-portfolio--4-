...
// Helper component to avoid prop drilling or messy hook usage in map
const ReadMoreButton = () => {
  ...
  return (
    <span className="text-xs font-medium text-slate-400 group-hover:text-slate-900 dark:group-hover:text-white transition-colors flex items-center gap-2">
      {content.initiatives.readMore} <ArrowRight size={12} className="group-hover:translate-x-1 transition-transform" />
    </span>
  );
}
...
export const Initiatives: React.FC<{ onNavigate?: (page: 'initiatives') => void }> = ({ onNavigate }) => {
  ...
  const tabs = [
    { id: 'media', label: content.initiatives.tabs.media, icon: Newspaper, activeColor: 'text-accent-red' },
    { id: 'events', label: content.initiatives.tabs.events, icon: Calendar, activeColor: 'text-accent-purple' },
  ];
    ...
    <section id="initiatives" className="py-24 px-6 bg-[#F5F5F7] dark:bg-brand-black border-t border-slate-200 dark:border-white/5 transition-colors duration-500 relative">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
          <div>
            <h2 className="font-display text-4xl font-bold mb-3 text-slate-900 dark:text-slate-100">{content.initiatives.title}</h2>
            <p className="text-slate-500 dark:text-slate-400 max-w-xl">
              {content.initiatives.subtitle}
            </p>
          </div>
            ...
            >
              {currentData.length > 0 ? (
                ...
                <div className="col-span-3 text-center py-20 bg-white/5 rounded-3xl border border-white/5">
                  <p className="text-slate-400">{content.initiatives.emptyState}</p>
                </div>
              )}
            </motion.div>
          )}
        ...
        <div className="flex justify-center mt-12">
          ...
          >
            <span className="relative z-10">{content.initiatives.exploreButton}</span>
